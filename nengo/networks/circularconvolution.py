...
class CircularConvolution(nengo.Network):
    ...
    def __init__(self, n_neurons, dimensions, invert_a=False, invert_b=False,
        ...
        self.config[nengo.Ensemble].update(ens_args)
        with self:
            self.A = nengo.Node(size_in=dimensions, label="A")
            self.B = nengo.Node(size_in=dimensions, label="B")
            self.product = Product(n_neurons, self.transform_out.shape[1],
                                   net=nengo.Network(label="conv"))
            ...
            nengo.Connection(self.product.output, self.output,
                             transform=self.transform_out, synapse=None)
